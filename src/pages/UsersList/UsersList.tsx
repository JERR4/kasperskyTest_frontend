...
const UsersList: React.FC = () => {
    ...
    function SlideTransition(props: SlideProps & { children: React.ReactElement }) {
        return <Slide {...props} direction="left">{props.children}</Slide>;
    }
    ...
    return (
        <Box className={'outerBox'}>
            ...
            <Box className="usersContent">
                        ...
                        <Stack spacing={2}>
                            {users.map(u => (
                                <Box key={u.id} p={2} border="1px solid #ccc" borderRadius={2}>
                                    <div><strong>Имя: </strong>{u.name}</div>
                                    <div style={{ overflowWrap: 'anywhere', wordBreak: 'normal' }}>
                                        <strong>Email: </strong>{u.email}
                                    </div>
                                    <div><strong>Возраст: </strong>{u.age}</div>
                                    <div><strong>Группа: </strong>{u.group?.name || '-'}</div>
                                </Box>
                            ))}
                        </Stack>
                    ) : (
                ...
                <Pagination page={page} total={total} limit={limit} onChange={setPage} /><Stack
                spacing={1}
